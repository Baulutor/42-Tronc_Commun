...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/23 18:48:58 by dbaule            #+#    #+#             */
/*   Updated: 2023/05/24 13:38:55 by dbaule           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void bresenham(int x, int y, float x1, float y1, t_data *data)
{
	...
	{
		mlx_pixel_put(data->mlx, data->win, (int)new_x, (int)new_y, data->color);
		new_x += x_step;
		new_y += y_step;
	}
}
...
static void	my_mlx_pixel_put(t_data *data, int x, int y, int color)
{
	...
	dst = data->win + (y * data->width + x * (24 / 8)); //bits_per_pixel
